...
class PhotoLogic extends Model
{
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        if ($spec_arr){
            ...
            $spec = Db::name('photo_spec')->column('id,spec_name');

            $specItem = Db::name('photo_spec_item')->column('id,item,spec_id');//规格项
            //$goods_sku = M('GoodsSku')->where('goods_id = '.$goods_id)->getField('item_path,price,store_count');//规格项
            $goods_sku = Db::name('photo_sku')->where(array("photo_id"=>$goods_id))->column('item_path,price,img');
...
//                $str .="<td><input name='item[$item_key]['price']' value='{$goods_sku[$item_key]['price']}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
                ...
                if (empty($goods_sku[$item_key]['img'])){

                    $str .="<td><ul class='photo_img'class='pic-list list-unstyled form-inline'></ul><input type='hidden'name='item[$item_key][photo_thumb]'id='photo$k'value=''><div><a href='javascript:uploadOneImage(\"图片上传\",\"#photo$k\")'><img src='/public/themes/admin_simpleboot3/public/assets/images/default-thumbnail.png'id='photo$k-preview'width='135'style='cursor: pointer'></a></div></td>";

                }else{

                    $url = cmf_get_image_preview_url($goods_sku[$item_key]['img']);

                    $str .="<td><ul class='photo_img'class='pic-list list-unstyled form-inline'></ul><input type='hidden'name='item[$item_key][photo_thumb]'id='photo$k'value=''><div><a href='javascript:uploadOneImage(\"图片上传\",\"#photo$k\")'><img src=$url id='photo$k-preview'width='135'style='cursor: pointer'></a></div></td>";
