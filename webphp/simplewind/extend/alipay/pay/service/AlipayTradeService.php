...
class AlipayTradeService {
	...
	function pagePay($builder,$return_url,$notify_url) {
		...
		//$response = $this->aopclientRequestExecute ($request,false); //针对vue的修改
	...
	function appPay($builder, $return_url, $notify_url) {
	    ...
	    //打印业务参数
	    $this->writeLog($biz_content);
	    $request = new AlipayTradeAppPayRequest(); //重点
	    $request->setNotifyUrl($notify_url);
	    $request->setReturnUrl($return_url);
	    $request->setBizContent($biz_content);
	    // 首先调用支付api
	 ...
	 * sdkClient
	 * @param $request 接口请求参数对象。
	 * @param $ispage 是否是页面接口，电脑网站支付是页面表单接口。2app
	 * return $response 支付宝返回的信息
	 */
	/*function aopclientRequestExecute($request, $ispage = 0) {
	    $aop = new AopClient();
	    $aop->gatewayUrl = $this->gateway_url;
	    $aop->appId = $this->appid;
	    $aop->rsaPrivateKey = $this->private_key;
