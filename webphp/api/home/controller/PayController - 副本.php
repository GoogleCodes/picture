...
class PayController extends RestBaseController
{
    ...
    public function wxnotify(){
        ...
        $myrequest = request();
        $ftext = '';
        $ftext = $ftext.$date.':'.print_r($_POST,true);
        $ftext = $ftext.'<br>'.print_r($myrequest,true).'<br>';
        ...
        /*if(!$_POST){
            ...
            $xarr = json_decode(json_encode($xmlstring),true);
            $easycof = config('easywechat');
            if($xarr['appid'] == $easycof['app_id'] && $xarr['mch_id'] == $easycof['payment']['merchant_id'] && $xarr['return_code'] == 'SUCCESS'){

                $out_trade_no = $xarr['out_trade_no'];
                ...
                $ftext = $ftext.'<br>'.print_r($xarr,true).'<br>';
                file_put_contents('./kkk.txt', $ftext);
                die();
        ...
        $app = new Application($options);
        file_put_contents('./kkk.txt', $ftext);
        file_put_contents('./kkk.txt', '00000000000',FILE_APPEND);

        try {
            
            file_put_contents('./kkk.txt', '111111',FILE_APPEND);
            $response = $app->payment->handleNotify(function($notify, $successful){

                $ftext = '进入了'.print_r($notify,true);
                //$ftext = '进入了'.print_r($successful,true);
                file_put_contents('./kkk.txt', $ftext,FILE_APPEND);
            ...
            file_put_contents('./kkk.txt', '3333',FILE_APPEND);
            $response->send(); // Laravel 里请使用：return $response;
            file_put_contents('./kkk.txt', '4444',FILE_APPEND);

        } catch (\Exception $e) {
            file_put_contents('./kkk.txt', '2222',FILE_APPEND);
            file_put_contents('./kaka.txt', print_r($e,true));
        }
