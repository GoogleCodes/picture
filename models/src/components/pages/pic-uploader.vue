	...
	<div class="img-area">
		<div class="image" @click="addPic($index)"
			...
			<input type="file" id="img{{ pid }}{{ $index }}" name="imgs[]"
				   @click="fileClick($index)" @change="getImage($index)" accept="image/jpeg,image/png,image/gif" />
...
<script>
	...
	export default{
		...
		methods: {
			...
			uploadImgToUpyun(url,file,policy,signature,index,domain,notify,params,thumb='') {
